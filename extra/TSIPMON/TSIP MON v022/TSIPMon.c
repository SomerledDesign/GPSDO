...
* version:
*-----------------------------------------------------------------------------*/
code uchar VersionMsg[]     = " TSIPMon v0.2.2 ";  // make 16 char long
code uchar LCDInitMsg[] 	= " www.ko4bb.com  ";  // make 16 char long
...
uchar LCD_STORAGE_CLASS lcdbuf[17];
...
* extern variables
*-----------------------------------------------------------------------------*/
extern char LCD_STORAGE_CLASS LCDBuf[];					// lcd_if.c
...
void PrimaryTiming( uchar TXRX_STORAGE_CLASS *RxBuf ){
	...
	mo = yr.u - 2000;
	UnsignedToAscii( (uint)mo, lcdbuf+14, 2 );
