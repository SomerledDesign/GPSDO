...
//
// Moon phase drawing code inspired by code from Mostafa Kaisoun
//
// Sun position code derived from the PSA algorithm from www.psa.es
//
...
int sunset;
int sunrise;
u08 leaped;           // flag set when leapsecond pending flag clears
...
#define MAX_SAT_COLOR 14   // this should be YELLOW - the sun
int sat_colors[] = {
   BROWN,
   BLUE,
   GREEN,
...
void write_log_header(FILE *file)
{
   ...
   if(unit_name[0]) {
      ...
      fprintf(file, "#   Unit type:     Trimble Mini-T\n");
   }
   else if(saw_ntpx) {
      fprintf(file, "#   Unit type:     Nortel NTPX\n");
   }
   else if(saw_nortel) {
      fprintf(file, "#   Unit type:     Nortel NTGS\n");
   }
   else if(luxor) {
      fprintf(file, "#   Unit type:     Luxor\n");
   }
   else {
   ...
   if(luxor) {
      fprintf(file, "#   HW version:    %u.%lu\n", (unsigned)luxor_hw_ver, (unsigned long)luxor_hw_rev);
      fprintf(file, "#   Serial number: %u.%lu\n", (unsigned)luxor_sn_prefix, (unsigned long)luxor_serial_num);
   }
   else {
      fprintf(file, "#   Serial number: %u.%lu\n", sn_prefix, (unsigned long) serial_num);
      fprintf(file, "#   Case s/n:      %u.%lu\n", case_prefix, (unsigned long) case_sn);
      fprintf(file, "#   Prodn number:  %lu.%u\n", (unsigned long) prodn_num, prodn_extn);
...
int i;
   ...
   if(log_written == 0) {  // write header each time log opens
      ...
      if(log_comments) {
         ...
         fprintf(file,"#  %02d:%02d:%02d %s   %02d %s %04d - interval %ld seconds\n", 
   ...
   if(log_errors && log_comments && spike_threshold && last_temperature && (ABS(temperature - last_temperature) >= spike_threshold)) {
...
//fprintf(file, "%ld: ", x);
   ...
   if(luxor) {
      fprintf(file, "%6lu%c%7.3f%c%7.3f%c%7.3f%c%7.3f%c%7.3f%c%7.3f%c%7.3f%c%7.3f%c%7.0f%c%7.0f%c%7.0f%c%7.0f%c%7.0f%c%7.3f \n", 
         (unsigned long) tow, csv_char, 
      ...
      fprintf(file, "%6lu%c%g%c%f%c%f%c%f%c%d \n", 
         (unsigned long) tow,  csv_char,
   ...
   if(log_comments && log_db && (file == log_file)) {
      for(i=1; i<=SUN_PRN; i++) {
...
{
   if(log_file && log_comments) {
      fprintf(log_file,"#\n");
      fprintf(log_file,"#! Leapsecond: %02d:%02d:%02d %s   %02d %s %04d - interval %ld secs\n", 
         hours,minutes,seconds, (time_flags&0x01)?"UTC":"GPS", day,months[month],year, log_interval);
      fprintf(log_file,"#\n");
   }
}
...
u16 change;
   ...
   if(log_comments && (discipline_mode != last_dmode) && (check_precise_posn == 0)) { 
      ...
      else if(discipline_mode == 5) fprintf(log_file, "Fast recovery");
...
int time_check(int reading_log, u16 interval, int hours,int minutes,int seconds)
{
long t;
int warn;
COORD row, col;
...
//if(log_file) fprintf(log_file, "#! (%d) pri=%02d:%02d:%02d  adj:%02d:%02d:%02d\n", warn, pri_hours,pri_minutes,pri_seconds, hours,minutes,seconds);
   ...
   if(warn) {  // we have a duplicate or missing time stamp
      ...
      if(log_comments && log_file && log_errors && (reading_log == 0)) {
...
//start_precision_survey();
    ...
    while(fgets(line, sizeof line, file) != NULL) {
        ...
        if((counter == 0) && (line[0] != '#') && (tim_file == 0) && log_comments) {  // it ain't no log file
           not_log:
...
//if(afile) fprintf(afile, "%.11le %.11le\n", pps_offset, osc_offset);
           ...
           if(luxor) {
              if(old_log_format) {   // old log format
                 sscanf(line, "%02d%c%02d%c%02d%c%ld%c%lf%c%lf%c%f%c%f%c%f%c%f%c%f%c%f", 
                 ...
                 sscanf(line, "%02d%c%02d%c%02d%c%ld%c%lf%c%f%c%lf%c%f%c%f%c%f%c%f%c%f%c%f%c%f%c%f%c%f%c%f%c%f", 
                   &hh,&ti[0],&mm,&ti[0],&ss,&ti[0], 
                   &tow,&ti[0], 
                   &vp,&ti[0],
                   &lux2,&ti[0],
              ...
              sscanf(line, "%02d%c%02d%c%02d%c%ld%c%lf%c%lf%c%f%c%f%c%d", 
                &hh,&ti[0],&mm,&ti[0],&ss,&ti[0], 
                &tow,&ti[0], 
                &vp,&ti[0],
                &vo,&ti[0],
...
if(undo_fw_temp_filter) {
   ...
   temperature = tvb;
           ...
           time_check(1, (u16) log_interval, hh,mm,ss);
//           if(time_check(1, log_interval, hh,mm,ss)) goto reload_done;
...
double eot()
{
#define DAYS (365.2422)
double ecc = 0.016709;    /* orbital eccentricity */
double plon = 1.347;      /* longitude of perihelion in radians */
double obliq = 0.4090928; /* obliquity in radians */
double jd;
double tz;
double eot_ofs;
double lon_ofs;
...
double x3, y3, z3;
   ...
   // returns the Equation of Time offset in minutes.

if(1) {
   jd = jdate(pri_year, pri_month, pri_day) - jdate(pri_year, 1, 1) - 1.7867; //2.310;
   jd += jtime(pri_hours,pri_minutes,pri_seconds);  // now
// jd += jtime(12,0,0);  // solar noon test

   tau = jd * ((2.0 * PI) / DAYS);

   theta = atan2(sqrt(1.0-ecc*ecc)*sin(tau), cos(tau)-ecc);

   /* first rotate clockwise in x-y plane by theta, corrected by plon */
   x1 = cos(theta-(plon-PI/2.0));
   y1 = sin(theta-(plon-PI/2.0));
   z1 = 0.0;
   ...
   /* lastly, set t equal to real time from tau and
      rotate counter-clockwise by t, corrected by plon */
   t = tau-ecc*sin(tau);
   x3 = cos(t-(plon-PI/2.0))*x2+sin(t-(plon-PI/2.0))*y2;
   y3 = -sin(t-(plon-PI/2.0))*x2+cos(t-(plon-PI/2.0))*y2;
   z3 = z2;

   eot_ofs = -atan2(y3, x3)*4.0/DEG_TO_RAD*DAYS/(DAYS+1.0);
}
else {
   jd = jdate(pri_year, pri_month, pri_day) - jdate(pri_year, 1, 1) + ((pri_hours-12.0)/24.0);  // approx day of perihelion - 12 days past winter equinox
   tau = jd * ((2.0 * PI) / DAYS);
   eot_ofs = (0.000075);
   eot_ofs += (0.001868*cos(tau));
   eot_ofs -= (0.032077*sin(tau));
   eot_ofs -= (0.014615*cos(2.0*tau));
   eot_ofs -= (0.040849*sin(2.0*tau));
   eot_ofs *= 229.18;
}

   tz = time_zone_hours * 60.0 + time_zone_minutes + time_zone_seconds / 60.0;
   lon_ofs = (-lon*RAD_TO_DEG*4.0) + tz;

sprintf(plot_title, "EOT:%g  jd:%g  tau:%g  lon:%g  lofs:%g  dt:%g  tz:%g", 
eot_ofs, jd, tau, lon, lon_ofs, eot_ofs-lon_ofs, tz);
...
double st;
   ...
   if(use_gmst || use_lmst) {
      ...
      if(solar_time) {
         st = jdate(pri_year, pri_month, pri_day) + jtime(pri_hours,pri_minutes,pri_seconds);  // now
         st += (eot() / (24.0 * 60.0));     // adjust for equation of time
         gregorian(st);   // convert Julian back to Gregorian

         pri_day     = g_day;     // set current time values to solar time
         pri_month   = g_month;
         pri_year    = g_year;
         pri_hours   = g_hours;        
      ...
      if(time_zone_set) {   // adjust date and time for time zone
         ...
         else if(pri_seconds > 60) { // >= 60 to ignore leap seconds
            pri_seconds -= 60;
            ++pri_minutes;
         }
...
double jd;
   ...
   // This routine plays a jaunty tune after a leap second
   ...
   if(leaped && (pri_seconds == LEAP_SECOND)) {
      leaped = 0;
      play_tune(LEAP_FILE, 1);
   }

   if(moon_file && (sun_el > 0.0)) {  // full moon starts during the day - play the Mr Moon song!
      jd = jdate(pri_year, pri_month, pri_day) + jtime(pri_hours,pri_minutes,pri_seconds);  // now
      if((jd == full_moon_jd) || (jd == blue_moon_jd)) {
         play_tune(MOON_FILE, 1);
      }
   }
   ...
   if(do_dump) {
      ...
      dump_screen(invert_video, top_line, s);
      do_dump = 0;
   }
...
//sprintf(plot_title, "Time busy %02X", system_busy);
   if(0 && system_busy) return;       // only set time when system is not too busy
...
int i;
   ...
   // calculates the julian date of the last new moon (to around 1 hour)
   jd = jdate(pri_year, pri_month, pri_day) + jtime(pri_hours,pri_minutes,pri_seconds);  // now

   last_p = moon_phase(jd);

   for(i=0; i<32*24; i++) {   // check back a month's worth of hours
      p = moon_phase(jd);
      if((p < 0.01) && (p > last_p)) {  // moon phase has started increasing
//gregorian(jd);
//sprintf(plot_title, "%d:%d:%d %d/%d/%d  jd=%lf", 
//g_hours,g_minutes,g_seconds, g_month,g_day,g_year, jd);
         return jd;
      }
      last_p = p;
      jd -= (1.0/24.0);       // back up an hour
   }
...
double ha;
   ...
   h = jtime(hours,minutes,seconds);
   d = jdate(year,month,day) + h - jdate(1999,12,31);
...
//   geocentric RA and Dec
   ...
   geodec = atan(ze / sqrt(xe*xe + ye*ye)) * RAD_TO_DEG;
...
// while(lst > 360.0) lst -= 360.0;
   ...
   dec = atan(ztop / sqrt(xtop * xtop + ytop * ytop));
   ...
   double xhor,yhor,zhor;
      ...
      x = cos(ha*DEG_TO_RAD) * cos(dec);
      y = sin(ha*DEG_TO_RAD) * cos(dec);
      z = sin(dec);
      ...
      moon_az = atan2( yhor, xhor ) + PI;
      moon_az *= RAD_TO_DEG;
      if(moon_az < 0.0) moon_az = 360.0 + moon_az;
      moon_el = asin(zhor); // = atan2( zhor, sqrt(xhor*xhor+yhor*yhor) );
      moon_el *= RAD_TO_DEG;
   }
   else {
   double cosha;
...
//sprintf(plot_title, "ra=%lf  dec=%lf   days=%lf h=%lf lst=%lf  ha=%lf  el=%lf  az=%lf", 
//ra,dec, d,h,  lst, ha, moon_el,moon_az);
}
...
double dParallax;
   ...
   // This routine calculates the sun position using the PSA algorithm

   // Calculate difference in days between the current Julian Day 
   // and JD 2451545.0, which is noon 1 January 2000 UTC
   dDecimalDays = jtime(hours,minutes,seconds);
   dElapsedJulianDays = jdate(year, month, day) + dDecimalDays;  // now
   dElapsedJulianDays -= (jdate(2000, 1, 1) + jtime(12,0,0));    // subtract ref time
   ...
   if(dRightAscension < 0.0) dRightAscension = dRightAscension + PI + PI;
   dDeclination = asin(sin(dEclipticObliquity)*dSin_EclipticLongitude);
   

   // Calculate local coordinates ( azimuth and elevation angles ) in degrees
   dGreenwichMeanSiderealTime = 6.6974243242 + 
           0.0657098283*dElapsedJulianDays 
           + (dDecimalDays * 24.0);
   dLocalMeanSiderealTime = (dGreenwichMeanSiderealTime*15.0*DEG_TO_RAD) + lon;
   dHourAngle = dLocalMeanSiderealTime - dRightAscension;
   dCos_Latitude = cos(lat);
   dSin_Latitude = sin(lat);
   dCos_HourAngle = cos(dHourAngle);

   dY = -sin(dHourAngle);
   dX = tan(dDeclination)*dCos_Latitude - dSin_Latitude*dCos_HourAngle;
   sun_az = atan2(dY, dX);
   if(sun_az < 0.0) sun_az = sun_az + PI + PI;
   sun_az = sun_az*RAD_TO_DEG;

   sun_el = (acos(dCos_Latitude*dCos_HourAngle*cos(dDeclination) 
          + sin(dDeclination)*dSin_Latitude));
   dParallax = (dEarthMeanRadius/dAstronomicalUnit) * sin(sun_el);   // apply parallax correction
   sun_el = 90.0 - (sun_el + dParallax) * RAD_TO_DEG;

   have_sun_el = 1;

//sprintf(plot_title, "%d:%d:%d  days=%lf   az=%lf  el=%lf  watch=%d  full=%d", 
//hours,minutes,seconds,  dElapsedJulianDays,  sun_az, sun_el,  plot_watch,zoom_screen); 
}
...
int i;
   ...
   // calculates the julian date of the last new moon (to around 1 hour)
   jd2 = jdate(pri_year, pri_month, pri_day) + jtime(pri_hours,pri_minutes,pri_seconds);  // now
   jd1 = jd2 - 1.0;   // yesterday
   jd3 = jd2 + 1.0;   // tomorrow

   p1 = moon_phase(jd1);  // get the moon illumination of the three consecutive days
   p2 = moon_phase(jd2);
   p3 = moon_phase(jd3);

   for(i=0; i<31; i++) {   // check back a lunar month's worth of days
      if((p3 >= p2) && (p1 >= p2)) goto check_hours;  // illumination is at a minimum

      jd3 = jd2;              // back up a day and try again
      jd2 = jd1;
      jd1 = jd1 - 1.0;
      p3 = p2;
      p2 = p1;
   ...
   check_hours:
// We have found the day of the last new moon,  Now check back each hour to
// find a more accurate illumination minimum.  We start checking a day plus 
// one hour earlier.
   jd3 = jd2 + 1.0 + 1.0/24.0;
   jd2 = jd3 - 1.0/24.0;
   jd1 = jd2 - 1.0/24.0;
   p1 = moon_phase(jd1);
   p2 = moon_phase(jd2);
   p3 = moon_phase(jd3);
   for(i=0; i<75; i++) {   // check back a day's worth of hours (plus a day on either side)
      if((p3 >= p2) && (p1 >= p2)) goto find_minute;
...
sprintf(plot_title, "no moon");
   return 0.0;

   find_minute:
// We have found the aday and hour of the last new moon,  Now check back each minute
// to find a more accurate illumination minimum.  We start checking an hour plus 
// one minute earlier.
   jd3 = jd2 + 1.0/24 + 1.0/24.0/60.0;
   jd2 = jd3 - 1.0/24.0/60.0;
   jd1 = jd2 - 1.0/24.0/60.0;
   p1 = moon_phase(jd1);
   p2 = moon_phase(jd2);
   p3 = moon_phase(jd3);
   for(i=0; i<183; i++) {   // check back a hours's worth of minutes (plus an hour on either side)
      if((p3 >= p2) && (p1 >= p2)) goto find_second;

      jd3 = jd2;
      jd2 = jd1;
      jd1 = jd1 - 1.0/24.0/60.0;
      p3 = p2;
      p2 = p1;
      p1 = moon_phase(jd1);
   }
   ...
   find_second:
   // algorithm is not accurate enough to bother with seconds
//gregorian(jd2);
//sprintf(plot_title, "new moon day: %d:%d:%d %d/%d/%d  jd=%lf  ph:%.10lf %.10lf %.10lf", 
//g_hours,g_minutes,g_seconds, g_month,g_day,g_year, jd2, p1,p2,p3);
   return jd2;
}
...
int gd;
   ...
   for(gd=1; gd<=dim[month]; gd++) {
      ...
      for(hour=0; hour<24; hour++) {
         ...
         if((last_p < 0.50) && (p >= 0.50)) {
//          moons[gd] = "Half Moon";  // really a quarter moon,  but that cramps the watchface
            moons[gd] = "Qtr Moon ";  // really a quarter moon,  but that cramps the watchface
         }
         if((last_p >= 0.50) && (p < 0.50)) {
//          moons[gd] = "Half Moon";  // really a quarter moon,  but that cramps the watchface
            moons[gd] = "Qtr Moon ";  // really a quarter moon,  but that cramps the watchface
...
double w, jd, l;
   ...
   t = (jdme - 2451545.0) / 36525.0;    // Julian centuries from 2000 (of equ/sol).
...
int height;
   ...
   #ifdef USE_X11
      set_x11_color(BLACK);
      XFillRectangle(display,win,gc, x,y, VCharWidth+VCharThickness,VCharHeight+VCharThickness);
      flush_x11();
   #endif
...
#define TIME_CHARS 8
   ...
   if(zoom_screen) {
      ...
      if(0 && plot_lla) return 1;
      if(zoom_lla) return 1;
...
int right;
   ...
   #ifdef USE_X11
      set_x11_color(BLACK);
      XFillRectangle(display,win,gc, aclock_x-ACLOCK_SIZE/2,aclock_y-ACLOCK_SIZE/2, 
...
int i;
   ...
   moon_color = YELLOW;
      ...
      moon_el = (moon_el - 90.0F) / 90.0F;
      moon_az += 90.0F;
      if(moon_az >= 360.0) moon_az -= 360.0;
      moon_x = (int) (moon_el * cos360((int)moon_az) * ACLOCK_R);
      moon_y = (int) (moon_el * sin360((int)moon_az) * ACLOCK_R);
   #endif
...
#define THETA_STEP 1
   ...
   line(aclock_x+stem_size, aclock_y-ACLOCK_R-stem_size, aclock_x+stem_size, aclock_y-ACLOCK_R, time_color);
// sprintf(plot_title, "aclock %d,%d, %d", aclock_x,aclock_y,ACLOCK_R);
}
...
#define MOON_SIZE  (ACLOCK_R/3)

#ifdef USE_X11
void x_arc(int xc,int yc, int d, int color, int fill)
{
int x,y;

   x = xc - (d/2);  // upper left corner of bounding box
   y = yc - (d/2);
   set_x11_color(color);
   if(fill) XFillArc(display,win,gc, x,y, d,d, 0,(360*64));
   else     XDrawArc(display,win,gc, x,y, d,d, 0,(360*64));
   flush_x11();
}
#endif


void draw_watch_moon()
{
int Xpos, Ypos, Rpos;
int Xpos1, Xpos2;
int y, x1,x2;
...
double synod;  // length of the synodic month
    ...
    jd = jdate(pri_year, pri_month, pri_day) + jtime(pri_hours,pri_minutes,pri_seconds); 
    moon_age = jd - new_moon_jd;     // calc the moon phase age in days since last new moon

    synod = (jd - 2451545.0) / 36525.0;
    ...
    moon_age = moon_age / synod;  // normalize moon age to 0.0 .. 1.0
    if(moon_age < 0.5) moon_age = moon_age + 0.50;
    else               moon_age = moon_age - 0.50;
    if(moon_age > 1.0) moon_age = 1.0;
    else if(moon_age < 0.0) moon_age = 0.0;
    moon_age *= 2.0;
    ...
    for(Ypos=0; Ypos<=MOON_SIZE/2; Ypos++) {  // draw the moon        
       ...
       if(moon_age < 1.0) {
          Xpos1 = 0 - Xpos;
          Xpos2 = (int)(Rpos - Xpos - (int) (moon_age*(double)Rpos));
       }
       else {
          Xpos1 = Xpos;
          Xpos2 = (int)(Rpos + Xpos - (int) (moon_age*(double)Rpos));
       ...
       x1 = aclock_x+moon_x+Xpos1;
       x2 = aclock_x+moon_x+Xpos2;
       if(x1 > x2) {   // clip off any part outside the drawing area
          t = x1;
          x1 = x2;
          x2 = t;
       }
       if(x1 <= (aclock_x-ACLOCK_SIZE/2+1)) x1 = aclock_x-ACLOCK_SIZE/2+2;
       if(x1 >= (aclock_x+ACLOCK_SIZE/2-1)) x1 = aclock_x+ACLOCK_SIZE/2-2;
       if(x2 <= (aclock_x-ACLOCK_SIZE/2+1)) x2 = aclock_x-ACLOCK_SIZE/2+2;
       if(x2 >= (aclock_x+ACLOCK_SIZE/2-1)) x2 = aclock_x+ACLOCK_SIZE/2-2;

       y = aclock_y+moon_y-Ypos;
       if(y > (aclock_y-ACLOCK_SIZE/2)) {
...
int x, y;
   ...
   if(map_and_watch) return;     // sun was part of the satellite map display
   ...
   az += 90.0F;
      ...
      if(sun_el >= 0.0) VFX_ellipse_fill(stage, x,y, ACLOCK_R/16,ACLOCK_R/16, palette[YELLOW]);
      else              VFX_ellipse_draw(stage, x,y, ACLOCK_R/16,ACLOCK_R/16, palette[YELLOW]);
   #endif
      ...
      if(sun_el >= 0.0) x_arc(x,y, ACLOCK_R/16, YELLOW, 1);
      else              x_arc(x,y, ACLOCK_R/16, YELLOW, 0);
   #endif

   #ifdef DOS
      x = ((aclock_x-TEXT_WIDTH/2  + (int) (el * cos360(az) * ACLOCK_R)) / TEXT_WIDTH) - 1;
      y = ((aclock_y-TEXT_HEIGHT/2 + (int) (el * sin360(az) * ACLOCK_R)) / TEXT_HEIGHT);

      if(sun_el >= 0.0) vidstr(y,x, YELLOW, SUN_STRING);
      else              vidstr(y,x, GREY,   SUN_STRING);
   #endif
}
...
void draw_watch(void)
{
   ...
   if(map_and_watch) ;
      ...
      if(plot_signals && zoom_screen) return;
...
void erase_azel() 
{
      ...
      set_x11_color(BLACK);
      if((zoom_screen == 0) && ((AZEL_ROW+AZEL_SIZE) >= PLOT_ROW)) {  // az/el map is in the normal plot window
         XFillRectangle(display,win,gc, PLOT_COL+PLOT_WIDTH,AZEL_ROW, SCREEN_WIDTH,AZEL_SIZE);
      }
      else {
         XFillRectangle(display,win,gc, AZEL_COL,AZEL_ROW, AZEL_SIZE,AZEL_SIZE);
      }
      flush_x11();
   #endif
...
int max_el, el_grid;
   ...
   for(az=AZ_INCR; az<=360; az+=AZ_INCR) {
      ...
      if(plot_el_mask && (plot_signals == 0) || (signals && ((plot_signals > 3)))) { 
      ...
      for(el=0; el<max_el; el+=el_grid) {  
         ...
         if(map_and_watch && (el == 0)) continue;
...
} sat_trail[SUN_PRN+1];  // one member for each sat
...
int az,el;
   ...
   for(prn=1; prn<=SUN_PRN; prn++) {       // for each satellite
      ...
      if(((az == 0) && (el == 0)) || (((sat_trail[prn].az[0]>>6) == az) && (sat_trail[prn].el[0] == el) && (i != SUN_PRN))) {  
...
u08 m;
   ...
   for(i=1; i<=SUN_PRN; i++) { // now draw the sat vectors
      if(sat[i].level_msg == 0x00) continue;
      if((i == SUN_PRN) && (sun_el == 0.0F) && (sun_az == 0.0F)) return;
      ...
      else if(marker_size < 3) marker_size = 3;
      if(SCREEN_WIDTH <= 800)  marker_size /= 2;   // scale markers to smaller screens
      
if(i == SUN_PRN) j = MAX_SAT_COLOR;  // !!!! kludge: force YELLOW for the sun

      #ifdef SAT_TRAILS
         // we draw the line first,  then the time marker dots
         // since the dots can be hollow and erase part of the line
         plot_sat_trail(grid_x,grid_y, grid_r,  i, az,el, sat_colors[j], 0);   // plot the satellite's path line 
         plot_sat_trail(grid_x,grid_y, grid_r,  i, az,el, sat_colors[j], 1);   // dot the satellite's path with time markers
      #endif
      ...
      if(sat[i].tracking > 0) {  //  mark sats that are actively tracked with a filled box
            ...
            if(map_and_sigs) {  // outline the sats in black
               VFX_ellipse_draw(stage, x,y,  marker_size+1, marker_size+1, palette[BLACK]);
               VFX_ellipse_draw(stage, x,y,  marker_size+2, marker_size+2, palette[BLACK]);
            }
            #ifdef DOS_MOUSE
               if(m) update_mouse();
            #endif
         #endif
         #ifdef USE_X11 
            x_arc(x,y, marker_size, sat_colors[j], 1);     
            if(map_and_sigs) {  // outline the sats in black
               x_arc(x,y, marker_size+1+1, BLACK, 0);
               x_arc(x,y, marker_size+2+1, BLACK, 0);
            }
         #endif
         #ifdef DOS
         ...
         #endif
         #ifdef USE_X11 
            x_arc(x,y, marker_size, sat_colors[j], 0);          
            if(map_and_sigs) {  // outline the sats in black
               x_arc(x,y, marker_size+1+1, BLACK, 0);
               x_arc(x,y, marker_size+2+1, BLACK, 0);
            }
...
int q3_q4_col, q1_q2_col;
   ...
   if(zoom_screen) show_title();
   no_x_margin = no_y_margin = 1;

   #ifdef SIG_LEVELS
      if(zoom_screen || ((SCREEN_HEIGHT >= 832) && (all_adevs == 0)) || ((SCREEN_WIDTH >= 1024) && (AZEL_ROW < PLOT_ROW))) {
         if(map_and_watch) label_circles(0, q2_row+3);
         else              label_circles(signals, q2_row+3);
      }

      if(map_and_watch) ;
      else if(signals && (plot_signals >= 4)) {
         if(zoom_screen || ((SCREEN_WIDTH >= 1024) && (AZEL_ROW < PLOT_ROW))) {
            q1_q2_col-=2;
            q3_q4_col-=2;
   ...
   j = 0;
   for(i=1; i<=SUN_PRN; i++) {  // first draw the sat PRNs in use
      if(sat[i].level_msg == 0x00) continue;
...
u08 show_sigs;
   ...
   if(show_map && show_sigs && (show_sigs != 3) && ((plot_areas == 1) || plot_lla)) { 
      map_and_sigs = 1;
   }

if(0) sprintf(plot_title, "areas:%d  shared=%d  maw:%d  mas:%d  azel:%d(%d,%d-%d)  sig:%d  lla:%d(%d,%d-%d)  watch:%d(%d,%d-%d)", 
plot_areas, shared_plot, map_and_watch, map_and_sigs,
plot_azel,AZEL_ROW,AZEL_COL,AZEL_SIZE, 
plot_signals, 
plot_lla,LLA_ROW,LLA_COL,LLA_SIZE,
plot_watch,WATCH_ROW,WATCH_COL,WATCH_SIZE 
);
...
char *s;
   ...
   if     (signals && (plot_signals == 1)) s = "Relative strength vs azimuth";
   else if(signals && (plot_signals == 2)) s = "1/EL weighted strength vs azimuth";
   else if(signals && (plot_signals == 3)) s = "Relative strength vs elevation";
   else if(signals && (plot_signals == 4)) s = "Signal strength vs az/el";
   else if(signals && (plot_signals == 5)) s = "Raw signal level data";
...
int az, el;
   ...
   fprintf(f,"#   %02d:%02d:%02d %s   %02d %s %04d\n", 
      hours,minutes,seconds, (time_flags&0x01)?"UTC":"GPS", day,months[month],year);
   fprintf(f, "#\n");
   fprintf(f, "#az el dbc\n");
...
int save_row, save_col, save_size;
   ...
   if(plot_watch && zoom_screen && (map_and_watch == 0) && (plot_signals == 0)) return;  // watch takes precedence over the azel map
   if(all_adevs && plot_signals && shared_plot) return;
   ...
   else if(plot_watch && (plot_signals == 0)) {  // both azel and watch want to be on the screen
      if(all_adevs) {
   ...
   if((plot_signals == 0) && (plot_areas == 2)) ;
...
//   else if(plot_signals && plot_azel && shared_plot && (all_adevs == 0)) {
   ...
   if(map_and_watch || map_and_sigs) ;
   else erase_azel();       // erase the old az/el plot

   azel_grid_color = AZEL_COLOR;
   if(map_and_watch || (map_and_sigs == 0)) {
...
void open_lla_file()
{
// if(lla_file == 0) lla_file = topen("LLA.LLA", "a");
   if(lla_file == 0) lla_file = topen("lla.lla", "w");
...
void start_precision_survey()
{
   ...
   if(log_comments && log_file) {
      fprintf(log_file, "# Precision %ld hour survey started.\n", do_survey);
   }
...
void stop_precision_survey()
{
   ...
   if(log_comments && log_file) fprintf(log_file, "# Precision survey stopped.\n");
