...
//                        !!! Note: directory changed for version 3.11 !!!
//
...
//
// 3.10 - 19 APR 2012   - Added support for Nortel NTGS55A compatible receivers.
//                        They should be automatically recognized and enabled. It
...
//
//
//  4.00 - 22 Jun 2012  - The sound files should now be placed in the heather.exe
//                        program directory.   They used to be in the
//                        c:\windows\media directory.
//
//                      - Improved solstice/equinox calculation algorithm
//
//                      - added /br command to set serial port baud rate
//
//                      - fixed a problem that occured when issuing a command
//                        line option from the keyboard (via the "/" keyboard
//                        command.  If the view interval was not 0,  then the
//                        interval would be changed.
//
//                      - Added support for days like Friday the 13th to the
//                        greetings calendar.
//
//                      - Added /ro=# command to add/subtract # seconds from
//                        the reported GPS date/time.  Also added support for
//                        automatic GPS week rollover compensation (if GPS year
//                        is less than 2012,  1024 weeks are added to the date
//                        unless the /ro offset has already been set by the user.
//                        If a rollover offset is in effect "ro" appears next to
//                        the GPS week display.  Automatic rollover compensation
//                        is activated only if the GPS UTC time offset is available.
//                        This keeps the rollover compensation from being applied
//                        during a power-up sequence.
//
//                      - Added moon phase image to analog watch display.
//                        Note that if parts of the moon image are outside the
//                        watch drawing area, they are clipped.  This could be
//                        mistaken for a bad moon phase image.  If the moon
//                        image was not clipped,  those parts would not be erased.
//
//                      - Added sun position to the satellite map display and
//                        the analog watch display. If the sun is below the 
//                        horizon,  it is shown as an un-filled circle.
//                        The trail of the sun position on the sat map display
//                        is limited to the last 6 hours.
//                        If you are logging the satellite consellation/signal 
//                        levels,  the sun az/el appears as PRN 33.
//
//                      - Added support for the Nortel NTPX oscillator.
//                        Note: the NTPX firmware does not do high-res temperature
//                        readings even if the "good" revision DS1620 temp
//                        sensor is installed.  
//
//                        Also, the NTPX and NTPB receivers do not appear to
//                        save the oscillator disciplining parameters in EEPROM.
//                        If you want to automatically use some non-default
//                        disciplining params,  you must specify them on the 
//                        command line or .CFG file using the "/u?" command
//                        line options.
//
//                      - If a Nortel receiver is detected and you do a !w, !c,
//                        or !h receiver reset command,  there is a 20 second
//                        delay before the recever is woken up again.  This is 
//                        needed since the receiver goes deaf during any reset
//                        command.
//
//                      - added !b command to send a BREAK to the receiver.  
//                        This causes most Trimble receivers to wake up and start
//                        responding to TSIP commands.
//
//                      - Modifed the watch and map displays to allow multiple
//                        maps (i.e. sat positions and signal levels) to be on
//                        the screen at the same time.  Also the analog watch
//                        display can overlay the satellite position map or
//                        the sat map can overlay the signal level display.
//                        Note that when the sat map overlays the signal level
//                        the display can be a bit ugly and the sats are not
//                        labeled. The sat trails can be hard to see.
//                        The GB command is labeled "Both map and adev tables"
//                        but is more properly "allow map displays in the plot
//                        area."
//
//                      - Changed the /tb command to toggle the labeling of the
//                        analog watch face.   /tb=Brand sets the watch brand
//                        name.  /tb= (with no brand name) sets the brand name
//                        to the day of the week.  /tb toggles the watch label.
//                        On previous versions /tb= set the brand name to "Trimble"
//
//                      - Added /gq command line option for toggling the signal
//                        quality map
//
//                      - added /tz command line option for rounding temperatures
//
//                      - added /bs command line option to adjust time by the
//                        solar Equation of Time.  The only indication that
//                        solar time is in use is on the analog watch display.
//                        The watch name/day of week will be "Solar".
//                        You can also select solar time by setting the time zone
//                        name to SST or SDT.
//
//                        Added the full IBM PC character set to the vector
//                        graphics character generator (previously it would
//                        only do : and the digits)
//
//                      - Clearing the plot queue no longer un-pauses the
//                        queue updates.  Previous versions enabled plot queue
//                        updates if they were disabled.
//
//                      - Reworked the structure of WinMain() so command
//                        line options are processed before the system
//                        abstraction layer (SAL) is initialized.
//
//                      - Rewrote the TSIP message parser.  It now buffers up
//                        the incoming message and only decodes it when the 
//                        end-of-message code is seen.  The program should no
//                        longer lock up if the serial port is disconnected
//                        in the middle of a message or if no receiver is 
//                        connected.
//
//                      - The number of possible plotted values has been
//                        increased.  Plots G0..G9 are now supported.  Previously
//                        only extra plots G1..G3 were supported.  This increase
//                        does double the size of the memory used by the plot
//                        queue.  Also added support for plotting values that 
//                        can be derived from the recorded data values.
//
//                      - Redid the code that displays the plot labels and
//                        cursor values.  It is now all centrally located in
//                        label_plots().   Previously it was spread all over creation.
//                        If a plot is disabled,  that plot's label and info 
//                        is not longer shown (previously it was greyed out). This
//                        allows more plots to be shown and labeled.  Also the 
//                        plot statistics values are shown aligned with the
//                        rest of the plot labels.
//
//                      - Added /lc command line option to not write any comments
//                        in the log file.  NOTE: to read a log file that was
//                        written without comments, the /lc command must be
//                        in effect,  otherwise the file will not be recognized
//                        as a log file!
//                      - Added /ls command line option to make the log file
//                        vlue separator a comma (default is a tab)
//                      - Added /ld command to write signal level comments in
//                        the log file.
// 
//                      - Added the WG and WI commands to do screen dumps of
//                        just the graph area and the plot labels.  The top part
//                        of the screen is not written.
//
//                      - If the mouse cursor is in the plot area and a screen
//                        dump is done, an 'X' marker is drawn in the plot at
//                        the cursor location.
//
//                      - The read file command can now load .CFG files and
//                        play .WAV files.
//
//                      - Added a "wait for alarm" option to the TA command when
//                        used in keyboard .SCRipt files.  If a "W" is placed on
//                        the TA command line,  the scipt file will be paused
//                        until the alarm triggers.   For example:
//                        "TA 10m w" will set a 10 minute egg timer.  The 
//                        execution of the script file will be paused for 10
//                        minutes.
//
//                      - Added MIN, MAX, and SPAN to plot statistic that can 
//                        be shown.  SPAN is the difference between MIN and MAX.
//
//                      - added /ty[=#] to display temperatures to # decial places
//
//                      - added elapsed time indication if the cursor is in
//                        plot area, the time from the start of the plotted data
//                        to the cursor is shown.
//
//                      - the VA (view all) command adjusts the view interval
//                        to display all the information in the plot queue.
//                        "Auto (DEL to stop)" instead of "Review (DEL to stop)...
//
//                      - the VT (view auto) command automatically adjusts the
//                        the view interval whenever incomming data causes the 
//                        plots to scroll left.  In View Auto mode, it says:
//                        "Auto (DEL to stop)" instead of "Review (DEL to stop)...
//
//                      - do srceen dump to "LEAP_SEC.GIF" if seconds is 60
//                        Also fixed adjust_tz() to show seconds value of 60
//                        (previous versions changed it to 00).
//
//                      - added support for compiling for Linux.  Uses X11 for
//                        screen / keyboard / mouse.  You will need to have the
//                        "libx11-dev" package installed to compile.
//
//                        For Linux, you specify the input device with command line
//                        option -id=device_name  or -999u says to use /dev/heather.
//                        -1u says to use /dev/ttyUSB0, -1 says to use /dev/ttyS0. etc.
//                        (devices from 0..99 are supported).  The number given
//                         on the command line is 1 greater than the linux device number!!!)
//
...
//  SetBreak(state)   - used to wake up Nortel GPSDO's
//
...
//  ID        units   ref scale  show  float  plot color
   ...
   {"TEMP",   "øC",   1000.0F,   1,    1,     TEMP_COLOR   },
   {"D\032H", "mHz",  1.0F,      0,    1,     ONE_COLOR    },
   {"DIF",    "x",    1.0F,      0,    1,     TWO_COLOR    },
   {"D-T",    "x",    1.0F,      0,    1,     THREE_COLOR  },
...
void config_lla_plots()
{
    // config G1 G2 and G3 plots to show lat/lon/alt
    plot[ONE].plot_center = (float) (lon * RAD_TO_DEG);
    plot[ONE].float_center = 0;
    plot[ONE].plot_id = "Lon";
    plot[ONE].units = deg_string;

    plot[TWO].plot_center = (float) (lat * RAD_TO_DEG);
    plot[TWO].float_center = 0;
    plot[TWO].plot_id = "Lat";
    plot[TWO].units = deg_string;

    plot[THREE].plot_center = (float) alt;
    plot[THREE].float_center = 0;
...
plot[LUX2].show_stat = SHOW_SPAN;
   ...
   if(log_file == 0) strcpy(log_name, "LUXOR.LOG");
   config_set = 1;
}
...
void find_sound_files()
{
   ...
   moon_file  = tune_exists(MOON_FILE, 1);   // see if moon tune sound file exists
...
char tune[MAX_PATH+1];
#ifdef USE_X11
   char shell_cmd[MAX_PATH+32];
#endif

   // This routine plays a sound file
   if(add_root) sprintf(tune, "%s%s", heather_path, file);
   else         sprintf(tune, "%s", file);
   ...
   #ifdef USE_X11
      sprintf(shell_cmd, "/bin/sh -c \"aplay %s\" &", tune);
      system(shell_cmd);  // play sound file asynchronously (non-blocking)
   #endif
}
...
/* !!!      scr_rowcol(TEXT_ROWS, 0);  move cursor off screen */
   ...
   #ifdef DOS_MOUSE
      if((text_mode == 0) && (zoom_screen == 0) && (mouse_disabled == 0)) {
...
#ifdef USE_X11  // X11 / Linux OS dependent I/O routines
...
void SetBreak(u08 on)
{   
   if(com_fd < 0) return;  // !!!! zork should also return if IP connected
...
void kill_com(void)
{
   // close the serial port file handle
   if(com_fd < 0) return;
...
printf("ip:%s  port:%d\n", IP_addr, portno);
   ...
   com_fd = socket(AF_INET, SOCK_STREAM | (SOCK_NONBLOCK*0), 0);
   if(com_fd < 0) {
...
printf("socket opened!\n");
   ...
   if(server == NULL) {
          sprintf(out, "ERROR, could not find IP host: %s\n", IP_addr);

      error_exit(20001, out);
      exit(0);
   }
printf("host found! %s\n", IP_addr);
   ...
   if(connect(com_fd,(struct sockaddr *) &serv_addr,sizeof(serv_addr)) < 0) {
...
int col, i;
   ...
   // initialize the serial device for asynchronous non-block communication
// !!! zork

   kill_com();   // in case COM port already open

   if((com_port == 0) && (usb_port == 0)) {
      //
      // TCP: In Windows, COM0 with process_com=TRUE means we're using TCP/IP 
      //
      if(process_com) {
   ...
   if(usb_port == 9999) {  // user set com device name with -id=xxx command line option
   }
...
// 
   com_fd = open(com_dev, O_RDWR | O_NOCTTY | O_NONBLOCK);
   if(com_fd < 0) {
      printf("Com open failed:%d\n", com_fd);
      printf("Do you have permission to access the serial device?\n");
      sprintf(out, "Can't open com port: %s", com_dev);
      error_exit(10001, out);
   }
   ...
   if(res_t_init) newtio.c_cflag |= (PARENB | PARODD);
...
col = 0;
while(0) {  // zork - debug test - blocking serial input
...
if(com_fd < 0) return FALSE; // !!!!!

   if(com_error & 0x01) {
printf("com avail err:%02X\n", com_error);
      return -100; // !!!! return 0;
   }
   if(next_serial_byte < rcv_byte_count) return 2000|next_serial_byte;  // we have already read and buffered the char

   rcv_byte_count = 0;
   next_serial_byte = 0;

   if(1 || (com_port != 0) || (usb_port != 0)) {  // zork COM port in use: read bytes from serial port
      flag = read(com_fd, &rcv_buf[0], sizeof(rcv_buf));
   ...
   while(rcv_byte_count == 0) {  // wait until we have a character
      if(1 || (com_port != 0) || (usb_port != 0)) {  // zork COM port in use: read a byte from serial port
         ...
         if(flag < 0) {  // !!!! com error or no serial data available - we should check errno()
            flag = rcv_byte_count = 0;
         }
         else rcv_byte_count = flag;
      }
...
int flag;
static unsigned char xmit_buffer[1024]; 
static long x = 0;   // Queue up data until ETX to reduce per-packet overhead 
...
double msecs;
   ...
   msecs = ((double) t.tv_sec) * 1000.0;
   msecs += ((double) t.tv_nsec) / 1000000.0;           
...
void Sleep(int time)
{
   time *= 1000;   // convet msecs to usecs
   while(time > 1000000) {
      usleep(1000000);
      time -= 1000000;
   }
...
void flush_x11(void)
{
   // flag that some X11 calls were made
   x11_io_done = 1;
}


u08 last_x11_color = 0xFF;
...
// VFX_io_done = 1;
   ...
   char *       window_name = "Lady Heather X11";
   char *       icon_name   = "Heather";
...
printf("setting properties\n");
   ...
   XDefineCursor(display,screen, cur);    // winz
...
printf("pixmap created\n");
   ...
   config_screen();  // re-initialize screen rendering variables
      ...
      clear_lla_points();
   #endif
printf("screen configed\n");   
   set_x11_color(BLACK);
   XFillRectangle(display,win,gc, 0,0,  SCREEN_WIDTH,SCREEN_HEIGHT); //winz
   flush_x11();
...
{
   if(color != last_x11_color) {
      last_x11_color = color;    
      XSetForeground(display, gc, palette[color]);
   }
}
...
{  
   ...
   #ifdef WIN_VFX
      VFX_string_draw(stage, col,row, vfx_font, " ", transparent_font_CLUT);
   #endif
   #ifdef USE_X11
      set_x11_color(BLACK);
      XFillRectangle(display,win,gc, col,row, TEXT_WIDTH,TEXT_HEIGHT);
      flush_x11();
   #endif
...
{  
   // erase the old character cell (and probably a little extra)
   #ifdef WIN_VFX
   y  VFX_string_draw(stage, col,row, vfx_font, " ", transparent_font_CLUT);
   #endif
   #ifdef USE_X11
      set_x11_color(BLACK);
      XFillRectangle(display,win,gc, col,row, TEXT_WIDTH,TEXT_HEIGHT);
...
s32 x;
   ...
   if(display == NULL) return;  // screen not initialized
...
if(!XCheckMaskEvent(display,~0, &report)) return 0;
//printf("event:%08lX\n", report.type);
    switch(report.type) {
      case Expose:
          if(report.xexpose.count != 0 ) break;
printf("Expose event seen\n");  // zork
          need_redraw = 1;
          return 1;
          break;
...
//printf("configure:%d %d  mouse:%d\n", width,height, this_button);  // zork 
          new_width = width;
          new_height = height;
          need_resize = 1;
          return 2;
...
//       else if(key ==  XK_F1)        f11_flag = 1;
//       else if(key ==  XK_F2)        f11_flag = 1;
//       else if(key ==  XK_F3)        f11_flag = 1;
//       else if(key ==  XK_F4)        f11_flag = 1;
//       else if(key ==  XK_F5)        f11_flag = 1;
//       else if(key ==  XK_F6)        f11_flag = 1;
//       else if(key ==  XK_F7)        f11_flag = 1;
//       else if(key ==  XK_F8)        f11_flag = 1;
//       else if(key ==  XK_F9)        f11_flag = 1;
//       else if(key ==  XK_F10)       f11_flag = 1;
         else if(key ==  XK_F11)       f11_flag = 1;
...
int p;
   ...
   // get the width of a VFX font character
...
int xx,yy;
   ...
   // draw a VFX font character using the dot routine 
   // (maybe useful if porting heather to a differnt operating system)
   ...
   #ifdef USE_X11
      set_x11_color(BLACK);
      XFillRectangle(display,win,gc, x,y, TEXT_WIDTH,TEXT_HEIGHT);
      flush_x11();
   #endif

   for(yy=0; yy<h; yy+=1) {
      for(xx=0; xx<w; xx+=1) {
         if(dot_font[p]) dot(x+xx,y+yy, attr);
#ifndef USE_X11
         else            dot(x+xx,y+yy, 0);
#endif
         ++p;
      }
   }
...
#endif
         else if(DOT_CHARS) {  // !!!! testing dot-by-dot character rendering
            px += dot_char(px,py, *chp, attr);
         }
         else {
...
void end_log()
{
   ...
   if(com_errors) {
      ...
      if(log_comments && log_file) fprintf(log_file, "#*** %lu COM PORT ERRORS!!!\n", (unsigned long) com_errors);
   }
   if(math_errors) {
      printf("*** %lu MATH ERRORS!!!\n", (unsigned long) math_errors);
      if(log_comments && log_file) fprintf(log_file, "#*** %lu MATH ERRORS!!!\n", (unsigned long) math_errors);
   }
   if(1 || bad_packets) {
      printf("%lu TSIP packets processed.  %lu bad packets.\n", (unsigned long) packet_count, (unsigned long) bad_packets);
      if(log_comments && log_file) fprintf(log_file, "#*** %lu TSIP packets processed.  %lu bad packets.\n", (unsigned long) packet_count, (unsigned long) bad_packets);
   }
...
void erase_plot(int full_plot) 
{
   ...
   if(zoom_screen) return;
      ...
      set_x11_color(BLACK);
      if(PLOT_COL > PLOT_LEFT) {
         XFillRectangle(display,win,gc, PLOT_LEFT,PLOT_ROW, PLOT_COL-PLOT_LEFT,SCREEN_HEIGHT-PLOT_ROW);
         flush_x11();
      }

      if(plot_background) { // slightly highlight the plot background color
         XFillRectangle(display,win,gc, PLOT_COL,PLOT_ROW, PLOT_WIDTH,SCREEN_HEIGHT-PLOT_ROW);
// RGB_TRIPLET(0,0,35));  // zork zoopy
      }
      else {  // black background
         XFillRectangle(display,win,gc, PLOT_COL,PLOT_ROW, PLOT_WIDTH,SCREEN_HEIGHT-PLOT_ROW);
      }
      ...
      // erase area to the right of the plot area
      if(full_plot && ((PLOT_COL+PLOT_WIDTH) < SCREEN_WIDTH)) {
         XFillRectangle(display,win,gc, PLOT_COL+PLOT_WIDTH,PLOT_ROW, SCREEN_WIDTH-(PLOT_COL+PLOT_WIDTH),SCREEN_HEIGHT-PLOT_ROW);
      }
      flush_x11();
   #endif
...
void erase_help() 
{
   ...
   #ifdef USE_X11
      set_x11_color(BLACK);
      XFillRectangle(display,win,gc, PLOT_LEFT,PLOT_ROW, SCREEN_WIDTH-PLOT_LEFT,SCREEN_HEIGHT-PLOT_ROW);
      flush_x11();
   #endif
...
void erase_lla() 
{
   ...
   #ifdef USE_X11
      set_x11_color(BLACK);
      XFillRectangle(display,win,gc, LLA_COL,LLA_ROW, LLA_SIZE,LLA_SIZE);
      flush_x11();
   #endif
...
void erase_screen()
{
   ...
   #ifdef USE_X11
      set_x11_color(BLACK);
      XFillRectangle(display,win,gc, 0,0, SCREEN_WIDTH,SCREEN_HEIGHT);
      flush_x11();
   #endif
...
long i;
   ...
   ++adev_q_size;  // add an extra entry for overflow protection
      ...
      if(adev_q == 0) {  // EMS memory not available,  use normal memort
         ...
         i *= (adev_q_size+1L);
         adev_q = (struct ADEV_Q HUGE_MEM *) (void HUGE_MEM *) halloc(i, 1);
      }
   #else
       i = 0L;
...
long i;
   ...
   if(plot_q) return; // plot queue memory already allocated
      ...
      if(plot_q == 0) {  // EMS memory not available,  try the plain stuff
         ...
         i *= (plot_q_size+1L);
         plot_q = (struct PLOT_Q HUGE_MEM *) (void HUGE_MEM *) halloc(i, 1);
      }
   #else
...
long i;
   ...
   if(hash_table) return;  // memory alrady allocated
       ...
       if(hash_table == 0) {  // EMS memory not available,  try the plain stuff
          ...
          i *= (HT_SIZE+1L);
          hash_table = (unsigned long HUGE_MEM *) (void HUGE_MEM *) halloc(i, 1);
       }
   #else
...
void config_azel()
{
   ...
   if(all_adevs && plot_lla && WIDE_SCREEN && (zoom_screen == 0)) {
...
// if(plot_azel || plot_signals || plot_watch) {
   if(plot_azel || plot_signals || plot_watch || plot_lla) {
...
//sprintf(plot_title, "share:%d  LLA: %d,%d,%d  XY=%d,%d  AZEL:%d,%d  ADEV:%d,%d  SCRW:%d",
//shared_plot, LLA_COL,LLA_ROW,LLA_SIZE,  LLA_X,LLA_Y, AZEL_COL,AZEL_ROW, ADEV_COL,ADEV_ROW, SCREEN_WIDTH);
   }
...
if(PLOT_ROW > TEXT_HEIGHT*26) PLOT_ROW = TEXT_HEIGHT*26;
   ...
   if(SMALL_SCREEN) {  // undersized screen
...
//sprintf(plot_title, "azel:%d,%d  lla:%d,%d  watch:%d,%d  adev:%d,%d", AZEL_ROW,AZEL_COL, LLA_ROW,LLA_COL, WATCH_ROW,WATCH_COL, ADEV_ROW,ADEV_COL);
...
vidstr(MOUSE_ROW+3, 0, WHITE, &blanks[0]);
   ...
   if(mouse_plot_valid) {
      ...
      if(top_line) {
         ...
         sprintf(out, "Cursor time: %02d:%02d:%02d %s  %s     View: %.1f min/div    ",
...
//             fmt_date(0) 
//         );
         sprintf(out, "Cursor time: %02d:%02d:%02d %s  %s  %s       ",
             pri_hours,pri_minutes,pri_seconds, 
             time_zone_set?tz_string:((q.sat_flags & UTC_TIME) ? "UTC":"GPS"), 
             fmt_date(0), 
             fmt_secs((view_interval * mouse_x)/plot_mag)
         );
      }
      no_x_margin = no_y_margin = 1;
      vidstr(MOUSE_ROW+2, 0, WHITE, &blanks[0]);  // erase the plot info area
   ...
   for(id=0; id<NUM_PLOTS+DERIVED_PLOTS; id++) {  // show the plot values
      ...
      if(id == OSC)  {
         ...
         else if(res_t) sprintf(out, "ref%c<%.1f%s>", c, val, plot[id].units); 
      ...
      if(id == OSC) {
         ...
         if(luxor) {
            ...
            sprintf(out, "%s: %.6f V", stat_id, val);
         }
      }
      else {
         val = get_stat_val(id);
      ...
      if(id == DAC) {
         if(luxor) {
            ...
            sprintf(out, "%s: %.6f V", plot[id].plot_id, q.data[id]/(float) queue_interval);
         }
      }
      else if(id == TEMP) {
   ...
   if(plot[FFT].show_plot && fft_scale) {
      ...
      else sprintf(out, "%s: DC blocked    ", plot[FFT].plot_id);
      no_x_margin = no_y_margin = 1;
      vidstr(MOUSE_ROW+2, 0, WHITE, &blanks[0]);  // erase the plot info area
      vidstr(MOUSE_ROW+2, MOUSE_COL+40, plot[FFT].plot_color, out);
      no_x_margin = no_y_margin = 0;
   }
#endif
   ...
   if(review_mode) {  // we are scrolling around in the plot queue data
      ...
      if(view_all_data == 1) {
         sprintf(out, "All (DEL to stop): %02d:%02d:%02d  %s %s         ", 
             pri_hours,pri_minutes,pri_seconds,  fmt_date(0),
             time_zone_set?tz_string:(q.sat_flags & UTC_TIME) ? "UTC":"GPS");
      }
      else if(view_all_data == 2) {
         sprintf(out, "Auto (DEL to stop): %02d:%02d:%02d  %s %s         ", 
             pri_hours,pri_minutes,pri_seconds,  fmt_date(0),
             time_zone_set?tz_string:(q.sat_flags & UTC_TIME) ? "UTC":"GPS");
      }
      else {
...
double v;
   ...
   if(graph_lla) {
      if(ONE < NUM_PLOTS)   q.data[ONE] += (float) (lon*RAD_TO_DEG);
      if(TWO < NUM_PLOTS)   q.data[TWO] += (float) (lat*RAD_TO_DEG);
...
void zoom_review(long i, u08 beep_ok)
{
   ...
   get_mouse_info();  // update cursor info in case we are not doing live data
   refresh_page();
}
...
#ifdef USE_X11
         if(*dot == '/') break;  // zork - !!!!! __linux__ needs help here
#else
         if(*dot == '\\') break;
#endif
         --dot;
...
int k;
   ...
   else if(new_user_view && (keyboard_cmd == 0) && user_view && queue_interval) {   // set user specified plot window time (in hours)
...
int j;
   ...
   com_port = 1;          // use COM1 or /dev/ttyS0
   usb_port = 0;
   com_fd = (-1);
   process_com = 1;
   first_msg = 1;        
   ...
   if(luxor) strcpy(log_name, "LUXOR.LOG");
   else      strcpy(log_name, "TBOLT.LOG");
   log_mode = "w";
...
#ifdef USE_X11
   strcpy(out, argv[0]);
   strcpy(help_path, basename(out));
   if(strstr(help_path,"luxor")) {
      luxor = 2;
   }

   // zork - get program name (help path) and executable path (heather_path)
   i = readlink("/proc/self/exe", out, sizeof(out));
   if(i < 0) readlink("/proc/curproc/file", out, sizeof(out)); 
   if(i < 0) readlink("/proc/self/path/a.out", out, sizeof(out)); 
   if(i >= 0) {
      strcpy(heather_path, dirname(out));
      strcat(heather_path, "/");
   }
...
{
   #ifndef WINDOWS
      config_program(argc, argv);  // Windows did this in WinMain()
   #endif
...
unsigned long x;
    ...
    show("");
    sprintf(out, "Valid startup command line options are:");
    show(out);
    show("   /0      - disable com port processing (if just reading a log file)");
#ifdef USE_X11
    show("   /1..99       - use /dev/ttyS(#-1) for TSIP input");
    show("   /1u..99u     - use /dev/ttyUSB(#-1) for TSIP input");
    show("   /999u        - use /dev/heather for TSIP input");
    show("   /id=dev_name - use Linux device dev_name for TSIP input");
#else 
    show("   /1..99  - use COM1 .. COM99 for TSIP input");
#endif
#ifdef TCP_IP
    show("   /ip=addr[:port#] - connect to TSIP server instead of local COM port)");
#endif
    ...
    show("   /br[=#] - set serial port baud rate.");
    show("   /bs     - adjust times by the solar Equation of Time");
    ...
    show("              J(el mask)  N(disable holiday greetings)   Q(signal quality map)");
    ...
    show("   /lc     - don't write any comments in the log file");
    show("   /ld     - write signal level comments in the log file");
    show("   /lh     - don't write timestamp headers in the log file");
    show("   /ls     - change log file value separtor to a comma");
    ...
    show("   /m0../m9[=#] - set scale factors of the other plots (units/divison)");
    ...
    sprintf(out, "   /q[=#]    - set size of plot Queue (default=%lu (%ld hours))", x, x/3600L);
    show(out);
    ...
    show("   /ro=#     - add # seconds to the GPS receiver date/time");
    ...
    show("   /tb       - do not label the analog watch face.");
    show("   /tb=string- set analog watch face brand name.");
    ...
    show("               Tries to play heather_chime.wav,  else uses alarm sound.");
    ...
    show("               Tries to play heather_song##.wav (##=minute)");
    ...
    show("   /tr       - show Rankine temperatures");
    ...
    show("   /ty[=#]   - show temperatures to # decimal places");
    ...
    show("   /vn[=#]   - Netbook (1000x540) Video screen (mode# unknown)");
    ...
    show("   /z[#][=val] - toggle or set graph zero line ref value (#=d,o,p,t,0..9)");
    show("                 (D)ac volts  (O)sc   (P)ps ns  (T)temp deg  (0)..(9)other plots");
    ...
    static char help_msg[32768] = {
         ...
         "   /br[=#]          - set serial port baud rate.\r\n"
         "   /bs              - adjust times by the solar Equation of Time\r\n"
         ...
         "                       J(el mask)  N(disable holiday greetings)  Q(signal quality map)\r\n"
         ...
         "   /ls              - change log file value separtor to a comma\r\n"
